...
- [ ] **GPU benchmark** (`benchmark_gpu.py`): Mirror the same JSON schema with
  `torch.cuda.Event` timing, NVIDIA power readings, and perun integration.
...
- [ ] **perun integration**: CPU energy via Intel RAPL on host for GPU benchmark.
