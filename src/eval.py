...
def process_large_patch(
    ...
    for y in range(0, height - model_patch_size + 1, stride):
        for x in range(0, width - model_patch_size + 1, stride):
            ...
            real_patch = real_large[:, :, y:y+model_patch_size, x:x+model_patch_size].contiguous()
            imag_patch = imag_large[:, :, y:y+model_patch_size, x:x+model_patch_size].contiguous()
...
def custom_inference(cfg: DictConfig):
    ...
    log.info(f"BPP: real={out_criterion_real['bpp_loss']:.6f}, imag={out_criterion_imag['bpp_loss']:.6f}, avg={(out_criterion_real['bpp_loss'] + out_criterion_imag['bpp_loss']) / 2:.6f}\n")
    ...
    log.info(f"BPP: real={patchified_results['metrics']['bpp_real']:.6f}, imag={patchified_results['metrics']['bpp_imag']:.6f}, avg={patchified_results['metrics']['bpp_avg']:.6f}")
    ...
    output_reflectivity_patchified = np.sqrt(0.5 * (output_real_patchified + output_imag_patchified))
    ...
    # Save images
    to_save = [real, imag, reflectivity, output_real, output_imag, output_reflectivity, output_real_patchified, output_imag_patchified, output_reflectivity_patchified]
    img_names = ["input_real", "input_imag", "input_reflectivity", "output_real", "output_imag", "output_reflectivity", "output_real_patchified", "output_imag_patchified", "output_reflectivity_patchified"]
    ...
    for img, name in zip(to_save, img_names):
        ...
        plt.imsave(str(inference_dir / f"{name}_{cfg_lambda}_{normalization}.png"), img_clipped, cmap='gray')
